...
def fetch_google_image(keyword):
    """Fetch the first image result from Google Images, fallback to placeholder."""
    try:
        ...
        with urllib.request.urlopen(req, timeout=3) as response:
            ...
            # Google often embeds thumbnail images starting with https://encrypted-tbn...
            urls = re.findall(r'<img[^>]*src="([^"]*encrypted-tbn[^"]+)"', html)
            if urls:
                return urls[0]
...
def render_route(df, t):
    ...
    for idx, (col, row) in enumerate(zip(cols, top_route.iterrows())):
        _, data = row
        with col:
            img_url = fetch_google_image(data["관심지점명"])
